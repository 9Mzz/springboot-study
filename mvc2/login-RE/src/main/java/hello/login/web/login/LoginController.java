...
public class LoginController {
  ...
  public String login(@Validated @ModelAttribute("loginForm") Login login,
    ...
    Member result = loginService.loginAction(login.getLoginId(), login.getPassword());
    log.info("login data = {}", result);
    if (result == null) {
      bindingResult.reject("loginError", "아이디 또는 비밀번호가 잘못되었습니다.");
      return "login/loginForm";
    }
    session.setAttribute(SessionConst.SESSION_MEMBER, result);
